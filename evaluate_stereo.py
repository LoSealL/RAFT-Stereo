...
def disp_init(disp_gt: torch.Tensor) -> torch.Tensor:
    ...
    # 4x downscale
    disp_rust = F.interpolate(disp_rust, scale_factor=0.25, mode="bilinear", align_corners=False)
    return disp_rust


@torch.no_grad()
def validate_synth(model, iters=4, mixed_prec=False):
    model.eval()
    val_dataset = datasets.SynthDataset(split="val")

    out_list, epe_list = [], []
    for val_id, ((_), image1, image2, flow_gt, valid_gt) in enumerate(val_dataset):
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()
        ...
        with autocast(enabled=mixed_prec):
            disp_rust = disp_init(flow_gt)
            _, flow_pr = model(image1, image2, iters=iters, flow_init=disp_rust, test_mode=True)
        flow_pr = padder.unpad(flow_pr).cpu().squeeze(0)
        ...
        image_out = out[val].float().mean().item()
        image_epe = epe_flattened[val].mean().item()
        logging.info(f"Synth Iter {val_id+1} out of {len(val_dataset)}. EPE {round(image_epe,4)} D1 {round(image_out,4)}")
        epe_list.append(image_epe)
        out_list.append(image_out)
