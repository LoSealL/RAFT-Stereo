...
class SynthDataset(data.Dataset):
    def __init__(self, root: str = "datasets/synth", split: Literal["train", "val"] = "train"):
        with open(Path(root) / f"{split}.txt", encoding="utf-8") as f:
            filelist = [line.strip() for line in f.readlines() if line.strip()]
        self.depth_gt = []
        self.image_left = []
        self.image_right = []
        for file in filelist:
            scene, frame = file.rsplit("_", 1)
            self.depth_gt.append(Path(root) / "depth_gt" / f"{scene}_gt_{frame}.png")
            self.image_left.append(Path(root) / "noir" / f"{scene}_left_{frame}.png")
            self.image_right.append(Path(root) / "noir" / f"{scene}_right_{frame}.png")
            assert self.depth_gt[-1].exists() and self.image_left[-1].exists() and self.image_right[-1].exists(), f"Missing files for {file}"

    def _depth_to_disp(self, depth: torch.Tensor) -> torch.Tensor:
        k = 57224
        b = 1.907
        return k / depth.float() + b

    def __len__(self) -> int:
        return len(self.depth_gt)

    def __getitem__(self, index) -> Tuple[list[torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        img0 = Image.open(self.image_left[index]).convert("RGB")
        img1 = Image.open(self.image_right[index]).convert("RGB")
        depth_gt = Image.open(self.depth_gt[index])

        img0 = torch.from_numpy(np.array(img0)).permute(2, 0, 1).float()
        img1 = torch.from_numpy(np.array(img1)).permute(2, 0, 1).float()
        depth_gt = torch.from_numpy(np.array(depth_gt)[:720]).long()
        disp = -self._depth_to_disp(depth_gt)[None]  # note: negative disparity
        valid = depth_gt < 65535
        # swap left right
        img0, img1 = img1.flip(-1), img0.flip(-1)
        disp = disp.flip(-1)
        valid = valid.flip(-1)
        return [], img0, img1, disp, valid.float()
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name.startswith("middlebury_"):
            ...
            new_dataset = SynthDataset(split=dataset_name.split("_", 1)[1])
            logging.info(f"Adding {len(new_dataset)} samples from Synth")
        train_dataset = new_dataset if train_dataset is None else train_dataset + new_dataset

    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, pin_memory=True, shuffle=True, num_workers=int(os.environ.get("SLURM_CPUS_PER_TASK", 6)) - 2, drop_last=True)
