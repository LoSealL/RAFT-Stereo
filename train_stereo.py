...
def fetch_optimizer(args, model):
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr, args.num_steps + 100, pct_start=0.01, cycle_momentum=False, anneal_strategy="linear")
...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            disp_rust = disp_init(flow)
            assert model.training
            flow_predictions = model(image1, image2, iters=args.train_iters, flow_init=disp_rust)
            assert model.training
